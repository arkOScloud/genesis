...
Genesis.ConfigController = Ember.ObjectController.extend({
  ...
  tzZones: function() {
    var self = this;
    var tzo = Genesis.Timezones.find(function(i) {
  ...
  actions: {
    updateTime: function() {
      var self = this;
      $.ajax({
        url: Genesis.Config.krakenHost+'/config/datetime',
        type: 'PUT',
        success: function(j) {
    ...
    save: function() {
      ...
      $.ajax({
        url: Genesis.Config.krakenHost+'/config/timezone',
        type: 'PUT',
